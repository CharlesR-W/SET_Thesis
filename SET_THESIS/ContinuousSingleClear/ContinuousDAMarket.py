...
class ContinuousDAMarket():
    ...
    def __init__(self, param_dict, rng):
        ...
        self.num_agents = param_dict["num_learners"] + param_dict["num_naive"]
        
        #initialize these arrays
        self.agent_Q_maxes = [None]*self.num_agents
        self.agent_MCs = [None]*self.num_agents
    ...
    def calculate_profits(self, P_clear, bids_arr):
        #given the market clearing price and the (cleared) bid array
        #breakpoint()
        reward_n = torch.zeros(size=[len(bids_arr)])
        #print(bids_arr)
        for bid in bids_arr:
    ...
    def clear_market_calculate_profits_no_update(self, act_n, PLOT_BIDS=False):
        ...
        reward_n = self.calculate_profits(P_clear, bids_arr)
    ...
    def clear_market(self, act_n):
        ...
        #breakpoint()
        sorted_acts = sorted(act_n, key = lambda x: x[act_P_idx]) #whacky way to do it but oke sure
