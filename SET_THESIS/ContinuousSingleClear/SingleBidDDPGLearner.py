...
class SingleBidDDPGLearner():
    ...
    def __init__(self, param_dict, env, agent_id=None):
        ...
        self.state_dim = env.get_agent_state_dim()

        self.policy_net = PolicyNet(self.state_dim, self.act_dim).to(device)
        self.Q_net = QNet(self.state_dim, env.get_agent_act_dim(), env.get).to(device)
        self.MQ_net = QNet(self.state_dim, env.get_agent_act_dim(), env.get).to(device)
    ...
    def learn_from_batch(self, batch):
        ...
        #breakpoint()
        #get the actions
        my_policy_acts = self.act(states, noise=False)
        other_acts = acts[0] #TODO: except for MINE!
        
        policy_loss_val = -1*torch.mean(self.Q_net(states, my_policy_acts)) #TODO should there be a detach on Q?  I want grad through a, but not through the Q params
