...
"""
import numpy as np
from ContinuousDAMarket import ContinuousDAMarket
...
def perform_run(env_param_dict, agent_param_dict, learn_interval, batch_size, num_episodes,  num_learners, random_seed, run_name, num_naive=0, is_MADDPG=True, PLOT_BIDS=True, has_supervisor=False,COURNOT=False,BERTRAND=False):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    num_agents = num_naive + num_learners
    ...
    for lv_episode in range(num_episodes):
        with torch.no_grad():
            ...
            act_n = [learner.act(state_n[:,lv_learner,:], noise=act_is_noisy, remember_unnoised_act=True) for lv_learner, learner in enumerate(learner_arr)] + [torch.tensor([[1,1]], device=device)] * num_naive
            act_n_tensor = torch.stack(act_n).squeeze(1)
            act_n = act_n_tensor.tolist() # [tensors] to [[[]]]
            ...
            terminated = True 
            next_state_n = [None]*num_learners
    ...
    if not is_MADDPG:
        for learner in learner_arr:
            learner.estimate_DDPG_deficit_history()
    """
    return learner_arr
