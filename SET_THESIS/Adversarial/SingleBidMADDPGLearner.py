...
class SingleBidLearner():
    ...
    def __init__(self, param_dict, env, agent_id=None, actor="MADDPG", has_supervisor=False, COURNOT=False, BERTRAND=False, IS_MASTER=False):
    ...
    def act(self, state, noise=True, remember_unnoised_act=True, act=None):
        ...
        if self.BERTRAND or self.IS_MASTER:
            act[:,0] = 1.0 #Q = Q_max
        elif self.COURNOT and not self.IS_MASTER:
            act[:,1] = 1.0 #P = MC
        ...
        if self.BERTRAND or self.IS_MASTER:
            ...
            act[:,1] = 1.0 #P = MC
            act[:,0] *= 0.1 #to get them in the olig regime
